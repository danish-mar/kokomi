...
- **A "laying out the document" animation while a PDF is written.** In place of a bare shimmer bar, the card shows a page whose regions slide and resize through four layouts — hero, two-column, sidebar, split — the way a page gets composed. Figures are filled surfaces with a small elevation shadow, copy is a heading bar over body bars on an 8px rhythm, and everything moves on the standard easing curve, holding each layout before transitioning. An eye button in the card header swaps between the animation and the markdown as it actually lands, and the choice survives the card being re-rendered on every chunk. The animation's phase does too — the card is rebuilt on every streamed chunk, and a new element restarts its CSS animations from the beginning, which would otherwise pin the whole thing to its first frame for exactly as long as the model kept writing.
