...
- **A "laying out the document" animation while a PDF is written.** In place of a bare shimmer bar, the card shows a filmstrip of page sheets being composed: a highlight travels from sheet to sheet, and each sheet's figure panels and copy lines draw themselves in as it arrives. An eye button in the card header swaps between the animation and the markdown as it actually lands, and the choice survives the card being re-rendered on every chunk. The animation's phase does too — the card is rebuilt on every streamed chunk, and a new element restarts its CSS animations from the beginning, which would otherwise pin the whole thing to its first frame for exactly as long as the model kept writing.
